...
def clean_label(t):
    """Comme clean(), mais sans insérer d'espace aux balises retirées.
    Évite l'artefact observé sur diplomatie.gouv.fr où certains titres sont
    à demi en gras ('<strong>P</strong>oliomyélite') : clean() y aurait laissé
    'P oliomyélite' (espace parasite au milieu du mot)."""
    t = re.sub(r"<[^>]+>", "", t or "")
    t = (t.replace("&amp;","&").replace("&nbsp;"," ").replace("&#039;","'")
           .replace("&rsquo;","'").replace("&laquo;","«").replace("&raquo;","»")
           .replace("&eacute;","é").replace("&egrave;","è").replace("&agrave;","à"))
...
def _section_block(html, start_pat, end_pats, max_len=6000):
    """Renvoie le HTML compris entre un titre <hN> correspondant à start_pat et le
    prochain titre correspondant à l'un des end_pats (ou max_len caractères si aucun trouvé)."""
    if not html:
        return ""
    m = re.search(r'<h[2-6][^>]*>(?:\s*<[^>]+>)*\s*' + start_pat +
                   r'\s*(?:<[^>]+>\s*)*</h[2-6]>', html, re.I)
    if not m:
        return ""
    start = m.end()
    end = start + max_len
    window = html[start:start+max_len*2]
    for ep in end_pats:
        me = re.search(r'<h[2-6][^>]*>(?:\s*<[^>]+>)*\s*' + ep, window, re.I)
        if me:
            end = start + me.start()
            break
    return html[start:end]

def _statut_vaccin(detail):
    """Classe le statut d'un vaccin à partir du texte de la puce (règles déterministes, pas d'IA)."""
    d = detail.lower()
    if "obligatoire" in d:
        return "Obligatoire selon provenance" if ("transit" in d or "provenance" in d) else "Obligatoire"
    if "exposition à risque" in d or "exposition a risque" in d:
        return "Si exposition à risque"
    if "milieu rural" in d or "éloigné" in d or "eloigne" in d:
        return "Recommandé (zones rurales)"
    if "fréquents ou prolongés" in d or "frequents ou prolonges" in d or "séjour prolongé" in d:
        return "Si séjour prolongé"
    if "en fonction des conditions" in d or "selon la dur" in d:
        return "Selon durée du séjour"
    if "recommand" in d or "conseill" in d:
        return "Recommandé"
    return "À évaluer avec votre médecin"

def fr_sante(html):
    """Extrait {vaccins:[{n,s}], risques:[nom...], note:str} depuis la page Santé FR.
    Repose sur la hiérarchie de titres observée sur diplomatie.gouv.fr (h3 'Risques sanitaires' /
    h4 'Vaccinations' / h4 'Frais d'hospitalisation...'), avec un repli neutre si une section manque."""
    if not html:
        return {"vaccins": [], "risques": [], "note": ""}

    bloc_risques = _section_block(html, r"Risques sanitaires",
                                   [r"Pendant le voyage", r"Liste des repr[ée]sentations"])
    groupeurs = ("maladies transmises", "autres maladies", "risques sanitaires")
    risques = []
    for h in re.findall(r'<h[2-6][^>]*>(.*?)</h[2-6]>', bloc_risques, re.DOTALL | re.I):
        nom = clean_label(h).strip(" *:")
        nl = nom.lower()
        if nom and len(nom) <= 40 and not any(g in nl for g in groupeurs) and nom not in risques:
            risques.append(nom)

    bloc_vac = _section_block(html, r"Vaccinations",
                               [r"Risques sanitaires", r"Pendant le voyage"])
    vaccins = []
    for li in re.findall(r'<li[^>]*>(.*?)</li>', bloc_vac, re.DOTALL | re.I):
        c = clean(li)
        m = re.match(r"([^:]{3,60}?)\s*:\s*(.+)", c)
        if m:
            vaccins.append({"n": m.group(1).strip(" *"), "s": _statut_vaccin(m.group(2))})

    bloc_frais = _section_block(html, r"Frais d.hospitalisation[^<]*",
                                 [r"Recommandations pour la sant", r"Vaccinations"])
    ps = [p for p in paras(bloc_frais) if not re.match(r"^afin de faire face", p, re.I)]
    note = ps[0] if ps else ""
    if note:
        note = note.rstrip(". ") + ". Assurance santé et rapatriement fortement recommandée."
    else:
        note = "Vérifier la couverture santé/rapatriement avant le départ ; infrastructures locales variables selon les régions."

    return {"vaccins": vaccins[:10], "risques": risques[:12], "note": note}
...
def scrape_country(iso, existing, visas_tn):
    ...
    sante = existing.get("sante", {"vaccins": [], "risques": [], "note": ""})
    if slug_fr:
        u = fr_urls(slug_fr)
        html_fr_sec = fetch(u["securite"])
        zones = fr_zones(html_fr_sec)
        date_fr_src, alertes_rss = fr_rss(u["rss"])
        html_fr_sante = fetch(u["sante"])
        if html_fr_sante:
            sante = fr_sante(html_fr_sante)  # ne réécrit pas l'existant si le fetch échoue
    ...
    src_str = source_date.strftime("%d %b %Y") if source_date else existing.get("sourceDateStr","—")
    print(f"      risk={risk} · {len(securite_tn)} sections · {len(zones)} zones · {len(sante.get('vaccins',[]))} vaccins/{len(sante.get('risques',[]))} risques santé · V{version}{' ✅' if changed else ''}")
